...
export default {
    ...
    methods:{
        ...
        goSearch(event){
            ...
            //利用事件委派存在的问题：
            //1：如何点击的一定要是a标签  即使是a标签，怎么区分层级
            //2：如何获取参数（1,2,3级分类跳转的名字和ID）
            //this.$router.push('/search')
            //1第一个问题：把子节点中的a加自定义属性
            let element = event.target;
            //获取到当前事件的节点[h3,a,dt,dl]，需要带有data-categoryName自定义属性的节点[a]
            //节点有一个dataset属性，可以获取节点的自定义属性和属性值
            let {categoryname,category1id,caterory2id,category3id} = element.dataset;
